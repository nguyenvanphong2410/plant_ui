...
import {MAX_STRING_ADDRESS, MAX_STRING_SIZE, VALIDATE_PASSWORD_REGEX, VALIDATE_PHONE_REGEX} from "@/configs";
...
const customValidateEmail = (value, helpers) =>
    new AsyncValidate(value, async function () {
        const user = await Admin.findOne({email: value, deleted: false});
...
export const register = Joi.object({
    ...
    phone: Joi.string()
        ...
        .custom(
            (value, helpers) =>
                new AsyncValidate(value, async function () {
                    const user = await Admin.findOne({phone: value, deleted: false});
